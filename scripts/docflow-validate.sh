...
validate_index_freshness() {
  ...
  # Freshness is judged against the real generator — never a second copy of it
  # here, which would call every map stale the moment the generator changes.
  map_script=""
  for candidate in "$SCRIPT_DIR/docflow-map.sh" "$TARGET/scripts/docflow-map.sh"; do
    [ -f "$candidate" ] && { map_script="$candidate"; break; }
  done
  if [ -z "$map_script" ]; then
    warn "INDEX.md: docflow-map.sh not found; skipped freshness check"
    return
  fi

  tmp="$(mktemp)"
  bash "$map_script" --stdout "$dr" > "$tmp" 2>/dev/null
